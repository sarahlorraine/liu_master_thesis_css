...
top4 <- components$csize %>% as_tibble() %>% rownames_to_column(., "component") %>% arrange(desc(value)) %>% top_n(5)
...
ggplot(gothenburg.subdiv.merged, aes(x=as.factor(regso), y=(avg_accessibility), size=nodes)) +
...
model1.rentals <- lm(avg_dijk_dist~foreign+nodes+total_households, 
             data = deso.demographics.rentals)
summary(model1.rentals)
model2.rentals <- lm(avg_dijk_dist~foreign+nodes+`Median income tkr`, 
             data = deso.demographics.rentals)
summary(model2.rentals)
model3.rentals <- lm(avg_dijk_dist~foreign+nodes+äganderätt+post_gymnasium_more_than_3, 
             data = deso.demographics.rentals)
...
ggplot(data = deso.demographics.rentals, aes(x=avg_dijk_dist, y=total_households)) + 
