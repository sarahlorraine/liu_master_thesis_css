...
#got_streets_net_old
### Bind the relevant streets and paths into one set 
# got_streets_alt <- rbind((streets$osm_lines %>% select(highway)), 
#                      main_streets$osm_lines %>% select(highway))

# Select just the "highway" value of the osmlines as the rest are not needed in the network
got_streets.lines <- got_streets$osm_lines %>% select(highway)
...
knn.sf.nodes <- nngeo::st_nn(x = charge_points.p2p.geo, 
...
knn.3 <- nn2(charge_points.long_lat.matrix, 
    query = got_streets_coords,
    k = 3,
    treetype = "kd",
    eps = 0)

#### FINDING NEAREST NODES FROM EACH CHARGE POINT #####

# Calculate closest nodes to each charge point using KNN 5
knn.3.nodes <- nn2(got_streets_coords, 
             query = charge_points.long_lat.matrix,
             k = 3,
             treetype = "kd",
             eps = 0)

# Calculate nearest nodes to each charge point using KNN 5 Radius
...
#closest_charge_old <- closest_charge
closest_charge <- data.frame()
for (i in 1:nrow(knn.3$nn.idx)) {
  #print(paste("running:", i,  "of", as.character(nrow(knn.3$nn.idx))))
  new_elements <- getMinKnn(i, knn.3$nn.idx, knn.3$nn.dists)
...
for (i in 1:nrow(knn.3.nodes[[2]])) {
  new_elements <- getMinKnn(i, knn.3.nodes$nn.idx, knn.3.nodes$nn.dists)
...
# View all edges in shortest path
test_path_graph %>%
  activate(edges) %>%
  as_tibble() %>%
  arrange(desc(length))
...
ggplot() +
 ...
 # geom_sf(data = got_streets_net %>% activate(nodes) %>% as_tibble() %>% st_as_sf(), size = 1, col = 'grey') +
  #geom_sf(data = got_streets_net %>% activate(edges) %>% as_tibble() %>% st_as_sf(), size = 1, col = 'grey') +
  # charge points
   geom_sf(data = got_streets_net %>% activate(nodes) %>% as_tibble() %>% filter(id %in% c(test_from_node, test_to_node)), lwd = 0.5, col = 'green') +
  # Road network nodes
 # geom_sf(data = joined.nodes %>% filter(id %in% c(test_from_node, test_to_node)), size=0.4) +
  # Label nodes
  #geom_sf_label(data= sample.nodes, label=sample.nodes$nodeID) +
  # Dijkstra's paths
   geom_sf(data = test_path_graph %>% activate(edges) %>% as_tibble() %>% st_as_sf(), lwd = 1, col = 'red')
  # Closest nodes
  # geom_sf(data = got_streets_net %>% activate(nodes) %>% filter(nodeID %in% test_to_node) %>% as_tibble() %>% st_as_sf(), col = 'blue', size = 1) +
  # Charge Points
  #geom_sf(data=charge_points.p2p.geo.snaps, col="red")
  # Coords All paths
  #coord_sf(xlim = c(11.986, 11.992),ylim = c(57.7170, 57.7135),expand = TRUE, clip = "on")
...
### !! Note: This functions takes about 12 hours to complete !! 
### Read the pre-generated data already exported from the csv:
### read_csv("data/dijkstras_per_point.csv")
distances_per_point.Subdivided <- read_csv("data/dijkstras_per_point_subdivided.csv")
...
#distances_per_point <- data_frame()
#distances_per_point.NoSubdivision <- data_frame()
distances_per_point.Subdivided <- data_frame()
...
for(i in 1:nrow(all_nodes)) {
  ...
  closest_cp <- charge_points.p2p.geo.nodes %>% 
    as_tibble() %>% 
  ...
  print(paste("charge point:", closest_cp$charge_id))
   ...
   distance_data <- path_graph %>%
    ...
    distances_per_point.Subdivided <- rbind(distances_per_point.Subdivided, new_distances)
  print("")
...
colnames(distances_per_point.Subdivided) <- c("from_node", "to_node", "distance")
view(distances_per_point.Subdivided)

distances_per_point.Subdivided %>% group_by(ifelse(distance == 0, "Failed", "Succeeded")) %>% summarize(n= n())
# Write result to csv
#write_csv(distances_per_point.Subdivided, "data/dijkstras_per_point_subdivided.csv")

# colnames(distances_per_point.NoSubdivision) <- c("from_node", "to_node", "distance")
# view(distances_per_point.NoSubdivision)
# distances_per_point.NoSubdivision %>% group_by(ifelse(distance == 0, "Failed", "Succeeded")) %>% summarize(n= n(), percent = n() / sum(n))
# #write_csv(distances_per_point.NoSubdivision, "data/dijkstras_per_point_no_subdivision.csv")
# 
# # Set column names
# colnames(distances_per_point) <- c("from_node", "to_node", "distance")
# view(distances_per_point)

# Save as csv for backup as the function takes 12 hours to run and you do not want to risk R crashing...
...
distances_per_point.sf.sub <- merge(distances_per_point.Subdivided,
...
distances_per_point.sf.sub <- distances_per_point.sf.sub %>%
  mutate(chargeId = closest_charge_sf$charge_id, 
         knn_distance = closest_charge_sf$distance,
         dijkstras_distance = distance) %>%
  select(from_node, to_node, chargeId, dijkstras_distance, knn_distance, everything(), -distance)

# Filter out nodes that are the same as the corresponding charge point as this distance is of course zero.
distances_per_point.sf.sub %>% view()
distances_per_point.sf.sub %>% filter(from_node == to_node)
...
ggplot() +
  ...
  geom_sf(data=filtered, size=0.5, aes(color=scale(dijkstras_distance, center = TRUE))) +
  # Charge points
...
distance_travel_matrix_dijk <- distances_per_point.sf.sub %>% 
  mutate(from_to = paste0(from_node, ":", to_node))

distance_travel_matrix_dijk <- merge(distance_travel_matrix_dijk, 
                                     distance_travel_matrix, 
                                     by="from_to") %>% 
  select(-length) %>% 
  mutate(length = dijkstras_distance,
         from_id = as.integer(from_id)) %>%
  st_join(., gothenburg.subdiv) %>%
  select(from_id, to_id, length, deso) %>% as_tibble()
...
got_streets_net.with_charge <- got_streets_net %>% 
  activate(nodes) %>% 
  mutate(charge_points = ifelse(id %in% charge_points.p2p.geo.nodes$nodeId, 1, 0))
...
joined_deso.nodes <- st_join(distances_per_point.sf.sub, gothenburg.subdiv, join = st_within, left=TRUE)
...
avg_dist_deso <- joined_deso.nodes %>% as_tibble() %>% 
  group_by(deso) %>% 
  summarise(nodes = n(),
            # remove NA values from the mean to avoid a NA result
...
ggplot() + 
  geom_sf(data = gothenburg.subdiv.merged, aes(fill=(log(avg_accessibility))), size=0.4) +
  # Charge points
  geom_sf(data=charge_points.p2p.geo.nodes.dist, col="blue") +
...
```{r plot_missings}
# Filter and plot the missing data_if any

zero_values <- joined_deso.nodes %>% 
  filter(dijkstras_distance == 0)

success <- joined_deso.nodes %>% 
  filter(dijkstras_distance > 0) %>% mutate(id = from_node) %>% as_tibble()
...
got_streets_nodes <- got_streets_net.merged %>% activate(nodes) %>% as_tibble() %>% mutate(id = as.numeric(id))
...
gothenburg.subdiv.merged %>% as_tibble() %>% 
  ggplot() + geom_density(aes(avg_accessibility)) + 
...
ggplot(gothenburg.subdiv.merged, aes(x=as.factor(regso), y=log(avg_accessibility), size=nodes)) +
  geom_point(alpha=0.7)

gothenburg.subdiv.merged %>% arrange(desc(avg_accessibility))
```
...
deso.demographics <- deso_aream2 %>%
  ...
  select(deso, regso, nodes, avg_accessibility, avg_dijk_dist, avg_euc_dist,  everything()) %>%
...
model1 <- lm(avg_dijk_dist~foreign+nodes+äganderätt+total_households, 
             data = deso.demographics)
summary(model1)
model2 <- lm(avg_dijk_dist~foreign+nodes+äganderätt+`Median income tkr`,
             data = deso.demographics)
summary(model2)
model3 <- lm(avg_dijk_dist~foreign+nodes+äganderätt+post_gymnasium_more_than_3, 
             data = deso.demographics)
summary(model3)
```

```{r lm_models_rental_areas, eval=TRUE, include=TRUE}
model1.rentals <- lm(avg_accessibility~foreign+nodes+total_households, 
             data = deso.demographics.rentals)
summary(model1.rentals)
model2.rentals <- lm(avg_accessibility~foreign+nodes+`Median income tkr`, 
             data = deso.demographics.rentals)
summary(model2.rentals)
model3.rentals <- lm(avg_accessibility~foreign+nodes+äganderätt+post_gymnasium_more_than_3, 
             data = deso.demographics.rentals)
...
ggplot(data = deso.demographics, aes(x=avg_dijk_dist, y=total_households)) + 
...
ggplot() + 
  geom_sf(data = gothenburg.subdiv.merged %>% filter(deso %in% deso.demographics.rentals$deso), aes(fill=log(avg_accessibility)), size=0.4) +
  scale_fill_distiller(palette = "Oranges")

# View data as table 
gothenburg.subdiv.merged %>% filter(deso %in% deso.demographics.rentals$deso) %>% arrange(avg_accessibility) %>% mutate(avg_accessibility = scale(avg_accessibility)) %>% view()
```
...
#### PCA and Correlations matrix
deso.demographics.pca <- deso.demographics.rentals %>% select(-deso, -regso, -nodes, -ages) %>% drop_na()
deso.demographics.normalized <- scale(deso.demographics.pca)
corr_matrix <- cor(deso.demographics.normalized)
```
