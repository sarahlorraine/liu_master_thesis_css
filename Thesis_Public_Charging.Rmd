...
# !! TODO - Don't just filter these without solving Olofstorp DeSO
...
charge_points.p2p <- p2p.data %>% filter(vehicleTypes.car == TRUE) %>% select(lat, lng)
...
nearest_nodes.osm <- st_nearest_feature(charge_points.osm, charge_point.coords)

charge_points.osm.nodes <- charge_points.osm %>% 
  st_set_geometry(st_geometry(charge_points.p2p.geo)[getChargePoint(nearest_nodes.osm)]) %>% 
  mutate(nodeId = getNodePoint(nearest_nodes.osm))

joined_charge_points <- merge(charge_points.p2p.geo.nodes %>% as_tibble(), charge_points.osm.nodes %>% as_tibble(), by="nodeId") %>% st_as_sf()

joined_charge_points %>% as_tibble() %>% view()
```
...
ggplot() +
  ...
  # Coords test path
  coord_sf(xlim = c(11.992, 12.020),
           ylim = c(57.709, 57.7231),
...
ggplot() + 
  ...
  # Charge points
  geom_sf(data=charge_points.p2p.geo.nodes, col="blue") +
...
model2 <- lm(avg_dijk_dist~foreign+nodes+äganderätt+`Median income tkr`,
             data = deso.demographics)
summary(model2)
model3 <- lm(avg_dijk_dist~foreign+nodes+äganderätt+post_gymnasium_more_than_3+education_missing, 
             data = deso.demographics)
summary(model3)
```
