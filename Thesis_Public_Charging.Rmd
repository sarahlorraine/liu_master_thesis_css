...
# Setup network to be compatibile for accessibility functions
distance_travel_matrix <- graph.charge_only %>% activate(nodes) %>% 
  as_data_frame() %>% 
...
joined.nodes <- joined.nodes %>% 
  mutate(knn_distance = knn_distance) %>% 
  mutate(distance_level = mapply(getDistanceLevel, knn_distance))

avg_dist_deso <- joined.nodes %>% as_tibble() %>% 
  group_by(deso) %>% 
  summarise(nodes = n(),
            avg_dist = mean(knn_distance) * 1000,
            avg_dd_accessibility = mean(distance_decay_accessibility)) %>%
...
ggplot(got.sub.new, aes(x=as.factor(regso), y=avg_dist, size=nodes)) +
...
model1.rentals <- lm(avg_dist~foreign+nodes, 
...
```{r setup_pca_matrix}
#### PCA and Correlations matrix
deso.demographics.pca <- deso.demographics.rentals %>% select(-deso, -regso, -nodes, -ages, -avg_dd_accessibility)
deso.demographics.normalized <- scale(deso.demographics.pca)
corr_matrix <- cor(deso.demographics.normalized)
```
