...
got_streets.lines <- got_streets.lines %>% select(highway, junction)
...
# Check the number of components in the network (isolated islands not connected to the main network)
#https://igraph.org/r/doc/components.html
components <- got_streets_net %>% components(., mode = c("weak", "strong"))

components$csize %>% as_tibble() %>% arrange(desc(value))

got_streets_net <- got_streets_net %>% activate(nodes) %>% mutate(component = components$membership)
# !! TODO - Don't just finlter these without solving Olofstorp DeSO
...
nearest_nodes <- st_nearest_feature(charge_point.coords, got_streets_net)

# Snap geometries of charge points to the network.
charge_points.p2p.geo.nodes <- charge_point.coords %>% 
  st_set_geometry(st_geometry(got_streets_net)[nearest_nodes]) %>% 
  mutate(nodeId = nearest_nodes, charge_id = charge_points.p2p.geo$charge_id)
```


```{r snap_points_to_nodes_OLD, include=FALSE}
...
ggplot() +
  ...
  geom_sf(data = charge_points.p2p.geo.nodes, size=0.9, color="red") +
  coord_sf(#lims_method = "geometry_bbox",
    xlim = c(11.71, 12.16), 
    ylim = c(57.58, 57.86), expand = TRUE, clip = "on") +
  labs(x="", y="") +
...
distances_per_point %>% group_by(ifelse(distance == 0, "Failed", "Succeeded")) %>% summarize(n= n())
