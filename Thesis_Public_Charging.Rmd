...
# got_streets <- rbind((streets$osm_lines %>% dplyr::select(highway)), 
#                      main_streets$osm_lines %>% dplyr::select(highway))
# 
...
got_streets.lines <- got_streets.lines %>% dplyr::select(highway, junction, maxspeed)
...
p2p.data <- p2p.data %>% dplyr::select(id, name, address, isFastCharge, powerRange, plugTypes, status, vehicleTypes, -`__typename`)
...
p2p.data <- p2p.data %>% na.omit(powerRange.min, powerRange.max) %>% dplyr::select(id, full_address, lon, lat, status,
...
charge_points.p2p <- p2p.data %>% filter(vehicleTypes.car == TRUE) %>% dplyr::select(-status, -vehicleTypes.bike, -vehicleTypes.motorbike)# %>% dplyr::select(lat, lng)
...
mode_cap <- mode(as.integer(joined_charge_points %>% filter(!is.na(capacity)) %>% pull(capacity)))

joined_charge_points <- joined_charge_points %>% 
  # Add capacity values and include the median (4) as default if NA
  mutate(capacity = as.integer(ifelse(is.na(capacity), mode_cap, capacity))) %>% 
  # Rename geometry and remove duplicates
  mutate(geometry = geometry.y) %>% 
  dplyr::select(-geometry.x, -geometry.y) %>% st_as_sf()
...
got_streets_net.knn <- cbind(got_streets_coords, closest_charge_sf) %>% 
  mutate(Lon = X, Lat = Y) %>% dplyr::select(-X, -Y)
...
#   dplyr::select(node_id, charge_id)
# 
...
#   dplyr::select(charge_id, geometry) %>%
#   as_tibble() %>%
#   st_as_sf(crs = 4326)
```
...
### !! Note: This functions takes about 4 hours to complete !! 
### Read the pre-generated data already exported from the csv:
#exposure_per_point <- read_csv("data/node_exposure_per_point.csv")
...
# define the weights to use. In this case time to destination
path_weights <- got_streets_net %>% activate(edges) %>% pull(time)
# Get the city center node for exposure calculation
city_center <- st_nearest_feature(central_station, got_streets_net)
city_center_node <- got_streets_net %>%
                    activate(nodes) %>%
                    filter(id == city_center) %>%
                    pull(id)

# Set the empty df for storing each result
exposure_per_point <- data_frame()
...
for(i in 1:nrow(all_nodes)) {
  ...
  current_node <- all_nodes[i,]$id
  print(paste("Running:", current_node))
  # get shortest path for the current node to the city center
    from_node <- got_streets_net %>%
                    activate(nodes) %>%
                    filter(id == current_node) %>%
                    pull(id) # ! It would be safer to use the vertex index here otherwise it can easily be broken if changes are made to the network graph, such as filtering as the shortest_path algorithm does not use the node attribute "id" col, but it's index.
    to_node <- city_center_node
    print(paste("from node:", from_node, "to node:",  city_center_node))
   ...
   time_data <- path_graph %>%
    ...
    print(paste("Distance:", distance_data, "Time", time_data))
    new_data <- tibble(from_node = from_node, to_node = city_center_node, time = time_data$time, distance = distance_data$length)
    exposure_per_point <- rbind(exposure_per_point, new_data)
  print("")
  print(paste(scales::percent(current_node / nrow(all_nodes)), "complete"))
  print("")
}
...
# Save as csv for backup as the function takes 6 hours to run and you do not want to risk R crashing...
#write_csv(exposure_per_point, "data/node_exposure_per_point.csv")
```
...
distances_per_point.sf <- distances_per_point.sf %>%
  mutate(chargeId = closest_charge_sf$charge_id, 
         ...
         exposure = exposure_per_point$time,
         travel_time = time,
...
Using a measure for culmulative count accessibility by distance decay. Operationalized as a negative exponential function for surrounding nodes
...
distance_travel_matrix <- closest_charge_sf %>% 
  ...
  dplyr::select(from_id, to_id, distance, from_to)
...
distance_travel_matrix_dijk <- distance_travel_matrix_dijk %>% 
  ...
  dplyr::select(from_id, to_id, distance, travel_time, deso) %>% as_tibble()
...
grouped_charge_points <- joined_charge_points %>% 
  ...
  as_tibble() %>% 
  dplyr::select(nodeId, capacity, -geometry) %>% 
  group_by(nodeId) %>% 
...
## NO LONGER USED. OLD MODEL OF ACCESSIBILITY
...
avg_dist_deso <- joined_deso.nodes %>% as_tibble() %>% 
  ...
  summarise(nodes = n(),
            ...
            avg_dijk_dist = mean(dijkstras_distance),
            avg_travel_time = mean(travel_time),
            avg_culm_accessibility = median(as.numeric(culmulative_accessibility))) %>%
  arrange(avg_culm_accessibility)
...
avg_dist_deso %>% mutate(avg_culm_accessibility = (avg_culm_accessibility)) %>% filter(deso == "1480C3220")
```
...
ggplot() + 
 # geom_sf(data = got_streets_net %>% activate(edges) %>% as_tibble() %>% st_as_sf(), size=0.4, col="grey") +
  ...
  #geom_sf(data = main_streets$osm_lines %>% dplyr::select(highway), size=0.4, col="black") +
  scale_fill_distiller(palette = "Oranges")
```
...
regso.list <- avg_dist_deso %>% dplyr::select(deso, regso)
...
deso.income <- deso.income %>% 
  dplyr::select(deso, `Median income tkr`) %>%
  filter(deso %in% regso.list$deso)
...
deso.education <- deso.education %>% 
  ...
  dplyr::select(deso, higher_education, lower_education, education_missing)

deso.housing <- deso.housing %>% 
  filter(deso %in% regso.list$deso) %>%
  dplyr::select(deso, hyresrätt, bostadsrätt, äganderätt) %>%
  filter(deso %in% regso.list$deso) %>%
  mutate(total = rowSums(.[, -1]),
         rental = hyresrätt / total,
         condominium = bostadsrätt / total,
         housing = äganderätt / total) %>%
  dplyr::select(deso, rental, condominium, housing)

deso.cars <- deso.cars %>%
  filter(deso %in% regso.list$deso) %>%
  mutate(cars_on_road = bilar_i_trafik) %>%
  dplyr::select(deso, cars_on_road)
...
deso.demographics <- deso_aream2 %>%
  ...
  mutate(people_per_household = pop / total_households) %>%
  mutate(density = pop / (area_m2 / 1000)) %>%
  dplyr::select(deso, regso, nodes, avg_dijk_dist,  everything()) %>%
  arrange(desc(avg_culm_accessibility))
...
# write_csv(deso.demographics %>% dplyr::select(-ages), "data/deso_demographics.csv")
# write.csv(deso.demographics.rentals %>% dplyr::select(-ages), "data/deso_demographics_rentals.csv")
...
```{r nodelevel_data_cleaning}
# test <- got_streets_net
# test <- test %>% activate("nodes") %>% mutate(eigen_centrality = eigen_centrality(got_streets_net)$vector)
# test.tibble <- test %>% activate("nodes") %>% as_tibble()
# plot(test.tibble$eigen_centrality)

# Closeness Centrality indicates how close a node is to the other nodes. 
# It is calculated as the average of the shortest path lengths of the node to the other nodes in the network.
# (8+ minutes to run)
start.time <- Sys.time()
closeness <- closeness(
  got_streets_net,
  vids = V(got_streets_net),
  mode = "all",
  weights = path_weights,
  normalized = FALSE,
  cutoff = -1
...
node_data <- joined_deso.nodes %>% as_tibble() %>%
  mutate(#capacity = as.numeric(grouped_charge_points[match(joined_deso.nodes$to_node, grouped_charge_points$nodeId),]$capacity),
         ...
         closeness = closeness,
         deso = factor(deso)) %>% 
  left_join(charge_land_use, by="id") %>%
  left_join(., deso.demographics, by="deso") %>%
  mutate(eigen_centrality = test.tibble$eigen_centrality) %>%
  rename(capacity = "charge_points") %>%
  dplyr::select(id, deso, regso, capacity, culmulative_accessibility, nodes, people_per_household, density, closeness, exposure, travel_time, area_m2, pop, rental, condominium, housing, cars_per_household, foreign, swedish, `Median income tkr`, higher_education, lower_education, total_households) %>% tibble()

# Scale the predictor variables
node_data <- node_data %>%
   mutate_at(c(7:length(colnames(node_data))), funs(c(scale(., center = TRUE))))
...
```{r all}
model1.accessibility <- glmer(formula = culmulative_accessibility ~closeness+foreign+cars_per_household+density+(1|deso),
              family="poisson",
              data=node_data)

model2.accessibility <- glmer(formula = culmulative_accessibility ~closeness+foreign*rental+density+(1|deso), 
              family="poisson",
              data=node_data)

model3.accessibility <- glmer(formula = culmulative_accessibility ~closeness+`Median income tkr`+cars_per_household+density+(1|deso), 
              family="poisson",
              data=node_data)

model4.accessibility <- glmer(formula = culmulative_accessibility ~closeness+`Median income tkr`*rental+density+(1|deso), 
              family="poisson",
              data=node_data)

fixef(model2.accessibility)["foreign"] + c(-2,2)*se.fixef(model2.accessibility)["rental"]
summary(model2.accessibility)
get_variance_random(model3.accessibility)
ranef(model1.accessibility)$deso %>% head(5)
plotREsim(REsim(model1.accessibility))
predictInterval(model1.accessibility)

model_list.accessibility <- list(model1.accessibility, model2.accessibility,  model3.accessibility, model4.accessibility)

stargazer(model_list.accessibility, type="text")
```

```{r all_models}
M1 <- glmer(formula = culmulative_accessibility ~closeness+foreign*rental+density+(1|deso), 
            family=poisson(),
            data=node_data)

M2 <- glmer(formula = culmulative_accessibility ~closeness+`Median income tkr`*rental+density+(1|deso), 
            family=poisson(), 
            data=node_data)

M3 <- lmer(formula = travel_time ~closeness+foreign*rental+density+(1|deso), 
          data=node_data)

M4 <- lmer(formula = travel_time ~closeness+`Median income tkr`*rental+density+(1|deso), 
           data=node_data)

summary(M1)

model_list.rentals <- list(M1, M2, M3, M4)

stargazer(model_list.rentals, type="text")
```
...
new_data <- deso.demographics %>% dplyr::select(foreign, `Median income tkr`, cars_per_household, nodes, housing) # Provide new data for prediction
...
ggplot() +
  geom_histogram(data = deso.demographics, aes(x=avg_culm_accessibility), fill="light blue") +
...
### Assumption checks
```{r}
culm_count.test1 <- accessibility::cumulative_cutoff(travel_matrix = distance_travel_matrix_dijk,
                                 land_use_data = charge_land_use,
                                 opportunity = 'charge_points',
                                 travel_cost = 'travel_time',
                                 cutoff = 5)

culm_count.test2 <- accessibility::cumulative_cutoff(travel_matrix = distance_travel_matrix_dijk,
                                 land_use_data = charge_land_use,
                                 opportunity = 'charge_points',
                                 travel_cost = 'travel_time',
                                 cutoff = 10)
...
ggplot(data = culm_count, aes(x=charge_points)) + geom_histogram(bins = 30) + 
  labs(#title="Cumulative count results per node", subtitle="Threshold = 8", 
       y="", x="Public chargers") + 
  theme_custom
ggplot(data = culm_count.test1, aes(x=charge_points)) + geom_histogram(bins = 30) + 
  labs(#title="Cumulative count results per node", subtitle="Threshold = 5", 
       y="", x="Public chargers") + 
  theme_custom
ggplot(data = culm_count.test2, aes(x=charge_points)) + geom_histogram(bins = 30) + 
  labs(#title="Cumulative count results per node", subtitle="Threshold = 10", 
       y="", x="Public chargers") + 
  theme_custom
```
...
deso.demographics %>% mutate(avg_accessibility = log10(avg_accessibility)) %>% dplyr::select(avg_accessibility) %>% max()
deso.demographics %>% mutate(avg_accessibility = log10(avg_accessibility)) %>% dplyr::select(avg_accessibility) %>% min()

# Confidence intervals
confint(model1.wt)

deso.demographics %>% arrange(desc(avg_culm_accessibility)) %>% dplyr::select(avg_culm_accessibility) %>% summarize(mean = mean(avg_culm_accessibility))
plot(model1)
```
...
png(file=paste0(dirname(getwd()), "/plots/robustness/homoscedasticity_m1.png"), width=622, height=519)
ggplot(data = node_data, aes(x = fitted(M1), y = resid(M1))) +
  geom_point() +
  geom_smooth(method = "loess", se = FALSE, linetype = "dashed") +
  labs(x = "Fitted Values", y = "Residuals") +
  #ggtitle("Residuals vs Fitted Values") +
  theme(text = element_text(size = 12, family = "Cambria")) + theme_custom
dev.off()

png(file=paste0(dirname(getwd()), "/plots/robustness/homoscedasticity_m2.png"), width=622, height=519)
ggplot(data = node_data, aes(x = fitted(M2), y = resid(M2))) +
  geom_point() +
  geom_smooth(method = "loess", se = FALSE, linetype = "dashed") +
  labs(x = "Fitted Values", y = "Residuals") +
  #ggtitle("Residuals vs Fitted Values") +
...
png(file=paste0(dirname(getwd()), "/plots/robustness/homoscedasticity_m3.png"), width=622, height=519)
ggplot(data = node_data, aes(x = fitted(M3), y = resid(M3))) +
  geom_point() +
  geom_smooth(method = "loess", se = FALSE, linetype = "dashed") +
  labs(x = "Fitted Values", y = "Residuals") +
  #ggtitle("Residuals vs Fitted Values") +
...
png(file=paste0(dirname(getwd()), "/plots/robustness/homoscedasticity_m4.png"), width=622, height=519)
ggplot(data = node_data, aes(x = fitted(M4), y = resid(M4))) +
  geom_point() +
  geom_smooth(method = "loess", se = FALSE, linetype = "dashed") +
  labs(x = "Fitted Values", y = "Residuals") +
  #ggtitle("Residuals vs Fitted Values") +
...
## QQplot ---------------------------------------------------------------------------------------
png(file=paste0(dirname(getwd()), "/plots/robustness/QQplot_res_m1.png"), width=622, height=519)
testUniformity(M1)
dev.off()

png(file=paste0(dirname(getwd()), "/plots/robustness/QQplot_res_m2.png"), width=622, height=519)
testUniformity(M2)
dev.off()

png(file=paste0(dirname(getwd()), "/plots/robustness/QQplot_res_m3.png"), width=622, height=519)
testUniformity(M3)
dev.off()

png(file=paste0(dirname(getwd()), "/plots/robustness/QQplot_res_m4.png"), width=622, height=519)
testUniformity(M4)
dev.off()

# Dispersion ------------------------------------------------------------------------------------
png(file=paste0(dirname(getwd()), "/plots/robustness/dispersion_m1.png"), width=622, height=519)
testDispersion(M1)
dev.off()

png(file=paste0(dirname(getwd()), "/plots/robustness/dispersion_m2.png"), width=622, height=519)
testDispersion(M2)
dev.off()

png(file=paste0(dirname(getwd()), "/plots/robustness/dispersion_m3.png"), width=622, height=519)
testDispersion(M3)
dev.off()

png(file=paste0(dirname(getwd()), "/plots/robustness/dispersion_m4.png"), width=622, height=519)
testDispersion(M4)
dev.off()

# Zero inflation --------------------------------------------------------------------------------
png(file=paste0(dirname(getwd()), "/plots/robustness/zero_inflation_m1.png"), width=622, height=519)
testZeroInflation(M1)
dev.off()
performance::check_zeroinflation(M1)

png(file=paste0(dirname(getwd()), "/plots/robustness/zero_inflation_m2.png"), width=622, height=519)
testZeroInflation(M2)
dev.off()

# Outlier test
png(file=paste0(dirname(getwd()), "/plots/robustness/outliers_m1.png"), width=622, height=519)
testOutliers(M1)
dev.off()

png(file=paste0(dirname(getwd()), "/plots/robustness/outliers_m2.png"), width=622, height=519)
testOutliers(M2)
dev.off()

png(file=paste0(dirname(getwd()), "/plots/robustness/outliers_m3.png"), width=622, height=519)
testOutliers(M3)
dev.off()

png(file=paste0(dirname(getwd()), "/plots/robustness/outliers_m4.png"), width=622, height=519)
testOutliers(M4)
dev.off()

# Independence Level 1
cor.test(residuals(M1), node_data$closeness)
cor.test(residuals(M2), node_data$closeness)
cor.test(residuals(M3), node_data$closeness)
cor.test(residuals(M4), node_data$closeness)

# Independence Level 2
test_data <- node_data %>% 
  group_by(deso) %>%
  select(deso, foreign, `Median income tkr`) %>% 
  unique()

cor.test(test_data$foreign, ranef(M1)$deso[, 1])
cor.test(test_data$`Median income tkr`, ranef(M2)$deso[, 1])
cor.test(test_data$foreign, ranef(M3)$deso[, 1])
cor.test(test_data$`Median income tkr`, ranef(M4)$deso[, 1])

# Simulation tests with residuals
...
l1resid <- residuals(M3)
head(l1resid)

node_data %>% 
  ggplot(mapping = aes(x = deso, y = l1resid)) +
  geom_point() +
  labs(x = "DeSO", y = "Residuals") + 
  theme_custom +
...
node_data %>% 
  ...
  geom_histogram(bins = 10) + labs(y="", x = "Level 1 residuals") + theme_custom
...
ggplot(data = deso.demographics, aes(y=avg_culm_accessibility, x=`Median income tkr`)) + 
  geom_point(aes(color=ifelse(foreign > 0.49, "foreign majority", "swedish majority"))) +
  labs(color="Swedish vs foreign")

ggplot(data = deso.demographics, aes(y=(avg_culm_accessibility), x=housing)) + 
  geom_point(aes(color=ifelse(foreign > 0.49, "foreign majority", "swedish majority"))) +
  labs(color="Swedish vs foreign")
```
...
deso.demographics.pca <- node_data %>% 
  rename(Exposure = "exposure",
         Population = "pop",
         `Higher education` = "higher_education",
         `Lower education` = "lower_education",
         `Charge point accessibility` = "culmulative_accessibility",
  ...
  dplyr::select(-id, -deso, -regso, -capacity, -nodes)
...
clusters <- deso.demographics %>% dplyr::select(avg_culm_accessibility, `Median income tkr`)
