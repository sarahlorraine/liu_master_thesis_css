...
ggplot(gothenburg.subdiv.merged, aes(x=foreign, y=(log10(avg_accessibility)))) +
  geom_bar(alpha=0.7)
...
model1 <- lm(log10(avg_accessibility)~foreign+cars_per_household,
             data = deso.demographics_na.rm)
summary(model1)

model2 <- lm(log10(avg_accessibility)~foreign+cars_per_household+`Median income tkr`,
             data = deso.demographics_na.rm)
summary(model2)
#log10(avg_accessibility)~foreign+avg_dijk_dist+pop+nodes+housing+cars_on_road
model3 <- lm(log10(avg_accessibility)~foreign+cars_per_household+`Median income tkr`+nodes, 
             data = deso.demographics_na.rm)
summary(model3)
```
...
model1.wt <- lm(log10(avg_accessibility)~foreign+cars_per_household,  
             data = deso.demographics_na.rm, weights=wt.1)
summary(model1.wt)

wt.2 <- 1 / lm(abs(model2$residuals) ~ model2$fitted.values)$fitted.values^2

model2.wt <- lm(log10(avg_accessibility)~foreign+`Median income tkr`+cars_per_household,  
             data = deso.demographics_na.rm, weights=wt.2)
summary(model2.wt)

wt.3 <- 1 / lm(abs(model3$residuals) ~ model3$fitted.values)$fitted.values^2

model3.wt <- lm(log10(avg_accessibility)~foreign+`Median income tkr`+cars_per_household+nodes,  
...
model1_wt.lv <- ivreg(log10(avg_accessibility)~foreign+cars_per_household+pop,  
             data = deso.demographics_na.rm, weights=wt.1)
