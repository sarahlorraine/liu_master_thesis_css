...
# Total P"P charge points
charge_points.p2p.geo %>% nrow()

# Get nearest nodes in the pre-snapped P2P charge point data
nearest_nodes.osm <- st_nearest_feature(charge_points.osm, got_streets_net)

charge_points.osm.nodes <- charge_points.osm %>% 
  st_set_geometry(st_geometry(got_streets_net)[nearest_nodes.osm]) %>% 
...
joined_charge_points <- joined_charge_points %>%
  ...
  rename(nodeId = "nodeId.x") %>% 
  arrange(nodeId) %>% 
  # Add the combined capacity values
  mutate(capacity = capacities$capacity) %>%
  st_as_sf()
...
distances_per_point <- data_frame()
#charge_points.p2p.geo.nodes %>% filter(charge_id == 167)
# Start timer
start.time <- Sys.time()
...
distance_travel_matrix_dijk <- distance_travel_matrix_dijk %>% 
  ...
  mutate(from_id = from_node, to_id = to_node, length = dijkstras_distance) %>%
...
got_streets_net.with_charge <- got_streets_net %>% 
  ...
  mutate(charge_points = as.numeric(
    ifelse(id %in% joined_charge_points$nodeId, 
                    joined_charge_points.df[match(id, joined_charge_points$nodeId),]$capacity,
                    0)
    )
  )  
...
ggplot() + 
  geom_sf(data = gothenburg.subdiv.merged, aes(fill=((avg_accessibility))), size=0.4) +
  # Charge points
  geom_sf(data=joined_charge_points, col="blue") +
