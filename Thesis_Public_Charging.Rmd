...
distance_travel_matrix <- closest_charge_sf %>% 
  ...
  dplyr::select(from_id, to_id, length, from_to)
...
distance_travel_matrix_dijk <- distance_travel_matrix_dijk %>% 
  ...
  dplyr::select(from_id, to_id, length, travel_time, deso) %>% as_tibble()
...
node_data <- node_data %>%
   mutate_at(c(7:length(colnames(node_data))), funs(c(scale(.))))
...
```{r rentals}
# Filter by majority rental areas
node_data_rentals <- node_data %>% filter(rental > 0.50)

model1.accessibility.rentals <- glmer(formula = culmulative_accessibility ~closeness+foreign+housing+density+(1|deso),
              family="poisson",
              data=node_data_rentals)

model2.accessibility.rentals <- glmer(formula = culmulative_accessibility ~closeness+foreign*rental+cars_per_household+density+(1|deso), 
              family="poisson",
              data=node_data_rentals)

model3.accessibility.rentals <- glmer(formula = culmulative_accessibility ~closeness+`Median income tkr`+housing+density+(1|deso), 
              family="poisson",
              data=node_data_rentals)

model4.accessibility.rentals <- glmer(formula = culmulative_accessibility ~closeness+`Median income tkr`+cars_per_household*density+(1|deso), 
              family="poisson",
              data=node_data_rentals)


summary(model2.accessibility.rentals)
get_variance_random(model3.accessibility.rentals)
ranef(model1.accessibility.rentals)$deso %>% head(5)
plotREsim(REsim(model1.accessibility.rentals))
predictInterval(model1.accessibility.rentals)

model_list.accessibility.rentals <- list(model1.accessibility.rentals, model2.accessibility.rentals, model3.accessibility.rentals, model4.accessibility)

stargazer(model_list.accessibility.rentals, type="text")
```

```{r multi_level}
model1.traveltime <- lmer(formula = (travel_time) ~closeness+foreign+cars_per_household+density+(1|deso),
              data=node_data,
              na.action=na.exclude)

model2.traveltime <- lmer(formula = (travel_time) ~closeness+foreign*rental+density + (1|deso), 
              data=node_data,
              na.action=na.exclude)

model3.traveltime <- lmer(formula = (travel_time) ~closeness + `Median income tkr`+cars_per_household+density+(1|deso), 
              data=node_data,
              na.action=na.exclude)

model4.traveltime <- lmer(formula = (travel_time) ~closeness + `Median income tkr`*rental+density + (1|deso), 
              data=node_data,
              na.action=na.exclude)

summary(model2.traveltime)
get_variance_random(model2.traveltime)
ranef(model2.traveltime)$deso %>% head(5)
plotREsim(REsim(model2.traveltime)) 
predictInterval(model2.traveltime)

model_list.traveltime <- list(model1.traveltime, model2.traveltime, model3.traveltime, model4.traveltime)

stargazer(model_list.traveltime, type="text")
...
ggplot(data = node_data, aes(x = fitted(model1.accessibility), y = resid(model1.accessibility))) +
  geom_point() +
...
cooksd <- cooks.distance(model1.mm.pois)
influential <- as.numeric(names(cooksd)[(cooksd > 4*mean(cooksd, na.rm=T))])  # influential row numbers
tail(node_data[influential, ])
...
vif(model1.accessibility)
vif(model2.accessibility)
vif(model3.accessibility)

vif(model1.traveltime)
vif(model2.traveltime)
vif(model3.traveltime)

## Normality of residuals
plot(model3.traveltime)
```
...
#### PCA and Correlations matrix
deso.demographics.pca <- deso.demographics.rentals %>% dplyr::select(-deso, -regso, -nodes, -avg_accessibility)
deso.demographics.normalized <- scale(deso.demographics.pca)
corr_matrix <- cor(deso.demographics.normalized)
```
...
#### PCA and Correlations matrix
deso.demographics.pca <- deso.demographics.owner.occupied %>% dplyr::select(-deso, -regso, -nodes, -avg_accessibility)
deso.demographics.normalized <- scale(deso.demographics.pca)
corr_matrix <- cor(deso.demographics.normalized)
```
