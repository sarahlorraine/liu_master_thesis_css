...
### !! Note: This functions takes about 12 hours to complete. Read the pre-generated data already exported from the csv:
...
for(i in 1:nrow(all_nodes)) {
  ...
  print(paste("for charge point:", closest_cp$charge_id))
    ...
    from_node <- got_streets_net %>%
                    ...
                    pull(id) # ! It would be safer to use the vertex indenx here otherwise it can easily be broken if changes are made to the network graph, such as filtering as the shortest_path algorithm does not use the node attribute "id" col, but it's index.
   ...
   distance_data <- path_graph %>%
    ...
    distances_per_point <- rbind(distances_per_point, new_distances)
...
# Merge with the road netowork data by from_node id to get the geometry values. Cast to sf network.
...
distances_per_point.sf <- distances_per_point.sf %>%
  mutate(chargeId = closest_charge$charge_id, 
...
# Filter the nodes to the GOT DeSOs to remove those that fall outside. This is just used for the initial plotting
...
distance_travel_matrix_dijk <- merge(distance_travel_matrix_dijk, 
                                     ...
                                     by="from_to") %>% 
  select(-length) %>% 
  mutate(length = scale(dijkstras_distance),
...
# Scale the results for ease of interpretation
negative_exp <- negative_exp %>% mutate(charge_points = charge_points)
...
negative_exp_dijk <- negative_exp_dijk %>% arrange((charge_points)) #mutate(charge_points = scale(charge_points))
...
ggplot() + 
  geom_sf(data = gothenburg.subdiv.merged, aes(fill=scale(avg_accessibility)), size=0.4) +
  # Charge points
  geom_sf(data=charge_points.p2p.geo.nodes.dist, col="blue") +
  scale_fill_distiller(palette = "Oranges")


## Code to likely be removed as it will not be used
