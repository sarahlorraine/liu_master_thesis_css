...
st_special <- data.frame(type = c("footway", "path","steps", "service", "cycleway", "track", "corridor", "via_ferrata", "pedestrian", "busway", "bus_guideway", "bridleway", "escape"))
non_vehicle <- data.frame(type = c("footway", "path", "steps", "cycleway", "corridor", "pedestrian","track", "bridleway"))
st <- subset(st, !type %in% st_special$type)
st <- as.character(st$type)
main_st <- as.character(main_st$type)
st_special <- subset(st_special, !type %in% non_vehicle$type)
st_special <- as.character(st_special$type)
...
### Bind the relevent streets and paths into one set 
got_streets <- rbind((streets$osm_lines %>% select(highway)), 
                     main_streets$osm_lines %>% select(highway)) %>% 
               rbind(., st_special$osm_lines %>% select(highway))
...
# Loop over each node and find the shortest path to a charge point
cp.test <- head(charge_points.p2p.geo.nodes)

for(i in 1:nrow(cp.test)) {
  # select closest nodes
  closest_nodes <- knn.5.nodes$nn.idx[i,] %>% as_tibble() %>% filter(value != cp.test[i,]$nodeId)
  print(paste("Start:",cp.test[i,]$nodeId))
  ...
  for(ii in 1:nrow(closest_nodes)) {
    ...
    print(paste("Distance:", path_graph %>%
            activate(edges) %>%
            as_tibble() %>%
...
from_node <- graph.merged %>%
  activate(nodes) %>%
  #filter(nodeID == "1018") %>%
  filter(nodeID %in% charge_points.p2p.geo.nodes$nodeId) %>%
  pull(nodeID)
...
to_node <- graph.merged %>%
  activate(nodes) %>%
  #filter(nodeID == "1019") %>%
  #filter((nodeID %in% non_charge_point$nodeID)) %>%
...
ggplot() +
  ...
  #geom_sf(data = st_special$osm_lines) +
  #geom_sf(data = joined.nodes, size=0.4, aes(color=distance_level)) +
  geom_sf(data = path_graph %>% activate(edges) %>% as_tibble() %>% st_as_sf(), lwd = 1, col = 'purple') +
  geom_sf(data = graph %>% activate(nodes) %>% filter(nodeID %in% cp.test[4,]$nodeId) %>% as_tibble() %>% st_as_sf(), col = 'green', size = 1) +
  geom_sf(data = graph %>% activate(nodes) %>% filter(nodeID %in% knn.5.nodes$nn.idx[4,]) %>% as_tibble() %>% st_as_sf(), col = 'red', size = 0.2) +
  #geom_sf(data=charge_points.p2p.geo.snaps, col="red") +
  scale_color_brewer(palette = "YlGnBu") +
  coord_sf(xlim = c(11.95, 12.1),
           ylim = c(57.68, 57.72),
           expand = TRUE, clip = "on")
