...
components$csize %>% as_tibble() %>% rownames_to_column(., "component") %>% count(value) %>% arrange((value)) %>% view()
...
joined_charge_points <- joined_charge_points %>% 
  # Add capacity values and include 1 as default if NA
...
ggplot() +
  ...
  # Dijkstra's paths
   ...
   # charge points
   geom_sf(data = got_streets_net %>% activate(nodes) %>% as_tibble() %>% filter(id %in% c(test_from_node, test_to_node)), size = 2.5, col = '#3B9AB2') +
   geom_sf_text(data = got_streets_net %>% activate(nodes) %>% as_tibble() %>% filter(id %in% c(test_from_node, test_to_node)), label = nodes.df$id, col="black", 
                fill="#94d2bd", 
                label.size = 1, 
                nudge_x = 0.001) +
  # Coordinates of path
  coord_sf(xlim = c(11.925, 11.945),
           ylim = c(57.6592, 57.6715),
...
grouped_charge_points <- joined_charge_points %>% count(nodeId) %>% as_tibble() %>% select(-geometry)
...
got_streets_net.with_charge <- got_streets_net %>% 
  ...
  mutate(charge_points = as.numeric(
    ifelse(id %in% grouped_charge_points$nodeId, 
                    grouped_charge_points[match(id, grouped_charge_points$nodeId),]$n,
                    0)
    )
  )  
...
ggplot(gothenburg.subdiv.merged, aes(x=as.factor(regso), y=(sqrt(avg_accessibility)), size=nodes)) +
...
# (avg_dijk_dist / 1000) changes the dependent variable from meters to kilometers
deso.demographics_na.rm <- deso.demographics %>% 
  # Remove the two NAs of Billdal
  filter(avg_accessibility > 0) %>% 
  rownames_to_column(., "i") %>% 
  # Remove outlier # 43
  filter(i != 43)

## For ease of readability it would be good to scale the average distance
model1 <- lm(log10(avg_accessibility)~foreign+pop+nodes+housing, 
             data = deso.demographics_na.rm)
summary(model1)
model2 <- lm(log10(avg_accessibility)~foreign+pop+nodes+housing+`Median income tkr`, 
             data = deso.demographics_na.rm)
summary(model2)
model3 <- lm(log10(avg_accessibility)~foreign+pop+nodes+housing+cars_on_road, 
             data = deso.demographics_na.rm)
summary(model3)
```
...
deso.demographics_na.rm %>% arrange(desc(avg_accessibility)) %>% select(avg_accessibility) %>% summarize(mean = mean(avg_accessibility))
plot(model1.wt)

# Homoscedasticity test
ggplot(data = deso.demographics_na.rm, aes(x = fitted(model1), y = resid(model1))) +
  geom_point() +
...
model3_wt.lv <- ivreg(log10(avg_accessibility)~foreign+total_households+nodes+housing+cars_on_road,  
             data = deso.demographics_na.rm, weights=wt.3)
...
ggplot(data = deso.demographics_na.rm, aes(x=log10(avg_accessibility), y=pop)) + 
...
gothenburg.subdiv.merged %>% filter(deso %in% deso.demographics.rentals$deso) %>% 
  arrange(avg_accessibility) %>% view()
```
...
deso.demographics.pca <- deso.demographics %>% 
  rename(Population = "pop",
         ...
         `Euclidian distance` = "avg_euc_dist",
...
clusters <- deso.demographics %>% select(avg_accessibility, `Median income tkr`)
