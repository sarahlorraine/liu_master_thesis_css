...
ggplot() +
  #geom_sf(data = got_streets_net %>% activate(nodes) %>% as_tibble() %>% st_as_sf(), col = nodes$component, size=0.5) + 
  #geom_sf(data = gothenburg.subdiv.merged, alpha=0.6, linewidth=0.6, colour="#8778D7", fill="#90C59B") +
...
ggplot() +
  geom_sf(data=deso_map_data, alpha=1, linewidth=0.3, colour="#505050", aes(fill=deso_map_data$pop)) +
...
ggplot() +
  geom_sf(data=gothenburg.subdiv.merged, alpha=0.6, linewidth=0.6, colour="dark gray", fill="#eeeeee") +
  geom_sf_label(data = utsatta_områden, label = utsatta_områden$neighbourhood, nudge_y = 0.009, alpha=1) +
...
ggplot() +
  ...
  geom_sf(data=gothenburg.subdiv.merged, aes(fill=(avg_travel_time)), linewidth=0.4) +
  #geom_sf_label(data = central_station, label = central_station$landmark, nudge_y = 0.012, alpha=0.8) +
  geom_sf(data = top10.travel, size=3) +
...
top10.rental <- got.sub.new.rentals %>% arrange(desc(avg_travel_time)) %>% head(10)  %>% as_tibble() %>% count(regso)
...
```{r map_all_desos}
top10.accessibility <- gothenburg.subdiv.merged %>% arrange((avg_culm_accessibility)) %>% head(10) %>% st_centroid()
...
ggplot() + 
  ...
  geom_sf(data = top10.accessibility, size=3) +
  #geom_sf_label(data = central_station, label = central_station$landmark, nudge_y = 0.012, alpha=0.8) +
  #geom_sf(data = central_station, size=2.8) +
...
deso.demographics %>% 
  ...
  select(-deso, -regso, -education_missing, -avg_culm_accessibility, -avg_exposure, -avg_dijk_dist, -avg_travel_time) %>% 
  stargazer::stargazer(type="latex", 
                       style="asr",
                       digits=2,
...
random_effects <- tidy(model1.accessibility) %>% filter(group %in% c("deso", "Residual")) #as.data.frame(VarCorr(model1.mm))[1,]
random_effects2 <- tidy(model2.accessibility) %>% filter(group %in% c("deso", "Residual")) #as.data.frame(VarCorr(model2.mm))[1,]
random_effects3 <- tidy(model3.accessibility) %>% filter(group %in% c("deso", "Residual")) #as.data.frame(VarCorr(model2.mm))[1,]
random_effects4 <- tidy(model4.accessibility) %>% filter(group %in% c("deso", "Residual")) #as.data.frame(VarCorr(model2.mm))[1,]

variance <- get_variance(model1.accessibility)
variance2 <- get_variance(model2.accessibility)
variance3 <- get_variance(model3.accessibility)
variance4 <- get_variance(model4.accessibility)

stargazer(model_list.accessibility,
          ...
          add.lines = list(#c("REML criterion", round(glmerMod(model1.mm.pois), 2), round(REMLcrit(model2.mm.pois), 2)),
                           c("Random Effects:"),
                           c("- DeSO Intercept", 
                             paste0(format(variance$var.intercept, digits = 3), " (", 
                                    format(random_effects[1,]$estimate, digits = 3), ")"), 
                             paste0(format(variance2$var.intercept, digits = 3), " (", 
                                    format(random_effects2[1,]$estimate, digits = 3), ")"),
                             paste0(format(variance3$var.intercept, digits = 3), " (", 
                                    format(random_effects3[1,]$estimate, digits = 3), ")"),
                             paste0(format(variance4$var.intercept, digits = 3), " (", 
                                    format(random_effects4[1,]$estimate, digits = 3), ")"))),
          align = FALSE,
          model.numbers = FALSE,
          column.sep.width = "-5pt",
          font.size = "small", 
          covariate.labels = c("Closeness centrality", "Foreign background", "Median income", "Housing", "Cars per household", "Urban density", "Cars per household:Urban density"),
...
```{r multi-model2}
summary(model3.traveltime)
random_effects.tt <- tidy(model1.traveltime) %>% filter(group %in% c("deso", "Residual")) #as.data.frame(VarCorr(model1.mm))[1,]
random_effects2.tt <- tidy(model2.traveltime) %>% filter(group %in% c("deso", "Residual")) #as.data.frame(VarCorr(model2.mm))[1,]
random_effects3.tt <- tidy(model3.traveltime) %>% filter(group %in% c("deso", "Residual")) #as.data.frame(VarCorr(model2.mm))[1,]
random_effects4.tt <- tidy(model4.traveltime) %>% filter(group %in% c("deso", "Residual")) #as.data.frame(VarCorr(model2.mm))[1,]

variance.tt <- get_variance(model1.traveltime)
variance2.tt <- get_variance(model2.traveltime)
variance3.tt <- get_variance(model3.traveltime)
variance4.tt <- get_variance(model3.traveltime)

stargazer(model_list.traveltime,
          type = "latex", # Choose output format (text, latex, html)
          title = "Multi-level Linear Regression Model Results",
          digits = 3,    # Number of digits to display
          add.lines = list(c("REML criterion", round(REMLcrit(model1.traveltime), 2), round(REMLcrit(model2.traveltime), 2), round(REMLcrit(model3.traveltime), 2)),
                           c("Random Effects:"),
                           c("- DeSO (Intercept)", 
                             paste0(format(variance.tt$var.intercept, digits = 3), " (", 
                                    format(random_effects.tt[1,]$estimate, digits = 3), ")"), 
                             paste0(format(variance2.tt$var.intercept, digits = 3), " (", 
                                     format(random_effects2.tt[1,]$estimate, digits = 3), ")"),
                           ...
                           # c("- DeSO (Residuals)", 
                           #    paste0(format(variance$var.residual, digits = 3), " (", 
                           #          format(random_effects[2,]$estimate, digits = 3), ")"), 
                           #    paste0(format(variance2$var.residual, digits = 3), " (", 
                           #          format(random_effects2[2,]$estimate, digits = 3), ")"))),
          align = FALSE,
          model.numbers = FALSE,
          column.sep.width = "-5pt",
          font.size = "small", 
          covariate.labels = c("Closeness centrality", "Foreign background", "Median income", "Housing", "Cars per household", "Urban density", "Cars per household:Urban density"),
          dep.var.labels = c("Travel time"),
          column.labels = c("Model 1", "Model 2", "Model 3", "Model 4")) # Align output

```

```{r modelcombined}
model_list.combined <- list(model2.accessibility, model4.accessibility, model2.traveltime, model4.traveltime)
